<?php
	...
	$q_update = "UPDATE bulkdata set status = '1'";
	mysqli_query( $connection,$q_update);
	echo "done update set 1";

$q_select = "SELECT * FROM bulkdata WHERE status = '1'  order by imei asc, st_server desc";
...
//foreach( mysqli_fetch_array($sql_select) as $row){
	...
	$d_select = "SELECT * FROM devices WHERE deviceid1 ='$imei'";
		$sql_devices = mysqli_query( $connection,$d_select);
		mysqli_error($connection);
		if(mysqli_num_rows($sql_devices) > 0){

$Select1 = "SELECT devicescol FROM devices WHERE deviceid1 = '$id'";
$exe_Select1 = mysqli_query( $connection,$Select1);
mysqli_error($connection);
$data_Select1= mysqli_fetch_assoc($exe_Select1);

if(!isset($data_Select1['devicescol'])){
	$devicescol = '2010-01-01 10:10:10';
}
else{
	$devicescol = $data_Select1['devicescol']; //--------------err
}
$Select3 = "SELECT id FROM sitara.positions order by id asc limit 1";
...
$uniqueb2 = intval($bbunique)+'1';
	...
	elseif($protocol == 'tw_sitara' || $protocol == 'al_shyma' || $protocol == 'Universal' || $protocol == 'topflay' || $protocol == 'anytracker'){
...
if ($dt_time > $devicescol ){ 
$newformat = date('Y-m-d H:i:s');
if($protocol != 'tw_sitara' && $protocol != 'al_shyma' && $protocol != 'Universal' && $protocol != 'topflay' && $protocol != 'anytracker' && $protocol != 'tw PTN'){
...
$Select2 = "SELECT uniqueId,name FROM devices WHERE deviceid1 = '$id'";
...
//echo $devicescol." device number<br>";

if($exe_Select2 == true && $pdivice_id != null){
	$insert_positions = "INSERT into sitara.positions(address,altitude,course,latitude,longitude,other,power,speed,time,valid,device_id,AlarmStatus,imileage,ikey,ireason,ireasoncode,syscode,vehicle_id,dtype,chk1,vlocation,overspeed,record_creation_time, recorddate)
	...
	$Select4 = "SELECT id FROM sitara.positions where device_id='$pdivice_id' order by time desc limit 1";
	$exe_Select4 = mysqli_query( $connection,$Select4);
	$data_Select4= mysqli_fetch_assoc($exe_Select4);
	$pt_id = $data_Select4['id'];
if($insert_positions == true ){
	$update_devices = "UPDATE devices SET devicescol = '$dt_time', latestPosition_id = '$pt_id' WHERE deviceid1 = '$id'";
	$basit1 = mysqli_query($connection,$update_devices);
	if($update_devices == true){
		echo $id."<br>";
	}
	mysqli_error($connection);
	...
	$pos_latest = mysqli_query($connection,"SELECT id FROM positions order by id desc limit 1;") or die(mysqli_error($connection));
	...
	$dev_latest = mysqli_query($connection,"SELECT uniqueId FROM devices order by uniqueId desc limit 1;") or die(mysqli_error($connection));
	...
	$sql_insert = mysqli_query($connection,"INSERT INTO  positions (id,altitude, course, latitude, longitude, power, speed, time, device_id, vlocation) 
	...
	//device date
	$sql_insert_dev = mysqli_query($connection,"INSERT INTO  devices (vstatus1,licensepn, latestPosition_id, name, uniqueId, simno, overspeed, devicescol, deviceid1,device_type, devicetype1,vehicle_make)  
	...
	//inser user 2
	$sql_user_two = mysqli_query($connection,"INSERT INTO  users_devices (users_id, devices_id, subacc_id, show_authority)   
	VALUES ('2' , '$dev_last' , '1' ,'0')") or die(mysqli_error($connection));
	$inserted_two = mysqli_query($connection,$sql_user_two);
	//inser user 3
	$sql_user_three = mysqli_query($connection,"INSERT INTO  users_devices (users_id, devices_id, subacc_id, show_authority)   
...
if($sql_select == true){
$q_delete = "DELETE FROM bulkdata WHERE status = '1'";
