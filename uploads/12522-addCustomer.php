...
// }
// $asset = "SELECT * FROM addasset";
$asset = "SELECT name , id from devices";
$result = mysqli_query($db, $asset);

$distributor = "SELECT * FROM `users` where privilege='Distributor'";
$result1 = mysqli_query($db, $distributor);

$admin = "SELECT * FROM `users` where privilege='Admin'";
$result2 = mysqli_query($db, $admin);
// $id = "SELECT id FROM `tc_users` ORDER BY id DESC LIMIT 1";
// $userid ;
// $tc_usersid = "SELECT id FROM `tc_users` ORDER BY id DESC LIMIT 1";
// $result = $db->query($tc_usersid);
...
if($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $admin_a = mysqli_real_escape_string($db,$_POST['admin_a']); 
    ...
    //     $sql = "INSERT INTO  addcustomer (`name`,`email`, `number`, `currency`, `username`, `password`,`billingAdd`,`paddress`,`securityNo`,`joinDate`,`billingProfile`,`assign`)
    //     VALUES ('$name', '$email', '$number',' $currency','$username','$password','$billingAdd','$paddress','$securityNo','$joinDate','$billingProfile','$assign')";
    //      //echo $assign . "<br>";
    //      //echo $sql;
    //     // echo $assign;
    ...
    //       }
    // }
    $sql =  "INSERT INTO  users (`name`,`privilege`,`login`,  `password`, `usersettings_id` ,`status`,`description`,`email`)
    VALUES ('$name', '$privilege', '$email', '112360d91e81336860629d08510570aa79b29a95d714be0b','1','1','$password','$email')";
    ...
    $userid ;
    $tc_usersid = "SELECT id FROM `users` ORDER BY id DESC LIMIT 1";
    $result = $db->query($tc_usersid);
    ...
    if($privilege==='End-User'){
        $sql1 = "INSERT INTO `distributor_end_user`(`distributor_id`, `end_user_id`) VALUES ('$distributor','$userid')";

        if (mysqli_query($db, $sql1)) {
            // echo '<script>alert("New distributor created successfully")</script>';
        } else {
    ...
    else if($privilege==='Distributor'){
        $sql1 = "INSERT INTO `admin_distibutor`(`admin_id`, `distributor_id`) VALUES ('$admin_a','$userid')";
    ...
    VALUES ('$userid','$assign')";

    if (mysqli_query($db, $sql1)) {
        // echo '<script>alert("New record'. $userid.' created successfully");window.location.reload;</script>';
//         echo "<script>
...
<body class="sidebar-noneoverflow starterkit">
                                        ...
                                        <div class="col-md-12">
                                            <form action = "addCustomer.php" method = "post">
                                                <div class="form-row mb-4">
                                                    ...
                                                    <div class="form-group col-md-6" onchange="mydistributor()">
                                                    ...
                                                    <div class="form-group col-md-6" style="display:none" id="show_dis">
                                                        <label for="inputAddress">Distributor</label>
                                                        
                                                        <select id="distributor" name="distributor" class="form-control selectpicker" >
                                                    ...
                                                    <div class="form-group col-md-6" style="display:none" id="show_ad">
                                                        <label for="inputAddress">Admin</label>
                                                        
                                                        <select id="admin_a" name="admin_a" class="form-control selectpicker" >
                                                            <option selected>Choose...</option>
    ...
    <script>
        function mydistributor(){
            var slect= document.getElementById("privilege").value;
            alert(slect)

            if(slect==='End-User'){
                document.getElementById("show_dis").style.display='block';
                document.getElementById("show_ad").style.display='none';

            }
            else if(slect==='Distributor'){
                document.getElementById("show_ad").style.display='block';
                document.getElementById("show_dis").style.display='none';
